...
public class H2PersistenceProvider implements PersistenceProvider {
	...
	// NOTE: There doesn't seem to be any advantage to using more compact key types
	...
	private SurrogateKey upsert(final TableDef table, final Row values) throws SQLException {
		...
		sql.append(") VALUES (");
		for (i = 0; i < table.getValueColumns().size(); i++) {
			if (i > 0)
				sql.append(",");
			sql.append("?");
		}
	...
	private synchronized void batchInsert(TableDef table, Row row) throws SQLException {
		...
		if (batchedRows.get(table).size() < batchSize)
			return;
		logger.info(() -> {
		...
		for (final ColumnDef column : table.getValueColumns()) {
			...
			if (refTable != null) {
				for (final ColumnDef refColumn : refTable.getValueColumns()) {
					...
					sql.append(" ");
					if (!refTable.getTableName().equalsIgnoreCase("PATH_DIM")) { // FIXME provide a nicer way to massage
																					// the column names
	...
	private void createIndex(final Statement stmt, final TableDef table, final String columnName, boolean hashIndex)
			throws SQLException {
	...
	/**
	 * A real connection pool seems pointless since every thread is going to be
	 * hammering on the db.
	 */
